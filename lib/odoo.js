...
class Odoo {
	constructor(config) {
		...
		if (typeof config.host !== "string") {
			throw new TypeError(
				`Expected \`host\` to be a \`string\`, got \`${typeof config.host}\``
			);
		}
	...
	connect(opts) {
		...
		return got
			.post(
				`${protocol}${config.host}:${config.port}/web/session/authenticate`,
				{ json: true, headers, body }
			)
			.then((res) => {
				if (res.body.error) {
				...
				return new OdooClient(res.body.result, {
					...
					sid: res.body.result.session_id
						? `session_id=${res.body.result.session_id}`
						: this.getSessionId(res.headers["set-cookie"]),
				});
			});
	}
