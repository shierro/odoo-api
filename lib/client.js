...
class OdooClient {
	...
	call(model, method, args, params = {}, argsWrapping = true) {
		if (argsWrapping) {
			args = [args];
		}
	...
	callButton(model, method, args, argsWrapping = true) {
		if (argsWrapping) {
			args = [args];
		}
		...
		return request(this._opts, "/web/dataset/call_button", body).then(
			response.parse
		);
	}
}
